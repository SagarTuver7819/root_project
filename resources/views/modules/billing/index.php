...
$actions = '';
if (can('billing.add')) {
    $actions .= '<a href="' . app_url('billing/create') . '" class="btn btn-primary"><i class="bi bi-plus-lg me-1"></i>Create Bill</a>';
}
if (can('outstanding.view')) {
    $actions .= '<a href="' . app_url('billing/outstanding') . '" class="btn btn-light"><i class="bi bi-exclamation-circle me-1"></i>Outstanding</a>';
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const table = RootsDataTable.init('#billingTable', {
    ...
    drawCallback: function () {
      $('#billingTable .bill-status-select').each(function () {
        paintBillStatus($(this));
      });
    }
  });

  $(document).on('change', '#billingTable .bill-status-select', function () {
    const $select = $(this);
